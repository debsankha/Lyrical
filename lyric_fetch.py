...
def main():
	artist=get('dcop amarok player artist').strip()
	title=get('dcop amarok player title').strip()
	print 'got info : %s'%artist
	if title =='':
		searchstr=get('dcop amarok player nowPlaying')+' lyrics'
	else :
	...
	for res in results:
		...
		if lyric=='':
			...
			print >> sh, '''dcop amarok contextbrowser showLyrics "${xml}"'''
			sh.close()
